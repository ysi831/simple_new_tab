const app = {
  ...
  methods: {
    ...
    toggleButtons(initial=false) {
      let isChecked = false;
      if (initial) {
        isChecked = app.data.locked;
      } else {
        isChecked = document.getElementById('btn-toggle').checked;
      }
  ...
  render() {
    ...
    const template = `
      <div class="column has-text-right">
        <div class="field">
          <input type="checkbox" id="btn-toggle" class="switch" ${(app.data.locked) ? "checked" : ''} name="switchRoundedDefault">
    ...
    document.querySelector('#btn-toggle').addEventListener("click",
      () => app.methods.toggleButtons(false)
    );
